...
begin
  ...
  if not exists(select 1 from pg_indexes where schemaname='public' and indexname='class_sessions_batch_date_uidx') then raise exception 'Missing batch/date uniqueness.'; end if;
  ...
  if to_regprocedure('public.find_or_create_batch_class_session(uuid,date)') is null or to_regprocedure('public.save_class_session_attendance(uuid,jsonb,boolean)') is null then raise exception 'Batch session RPCs are missing.'; end if;
end $$;
rollback;
