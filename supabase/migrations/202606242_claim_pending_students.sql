...
begin
  ...
  loop
    ...
    if membership_row.role = 'coach' then
    ...
    elsif membership_row.role = 'student' then
      ...
      if linked_student.id is not null then
        insert into public.audit_logs (
          ...
          auth.uid(),
          membership_row.academy_id,
          'student.linked_to_profile',
          'student',
          linked_student.id,
          jsonb_build_object('email', linked_student.email, 'status', 'pending_login'),
          to_jsonb(linked_student)
        );
      end if;
    end if;
  end loop;
